...
import { SECS_PER_QUESTION, API_BASE_URL, DIFFICULTY, HIGHSCORE_STORAGE_KEY } from "../constants/config";
    ...
    switch (action.type) {
        ...
        case ACTION_TYPES.START: {
            ...
            return {
                ...
                secondsRemaining: Math.round(state.questions.length * SECS_PER_QUESTION * multiplier),
        ...
        case ACTION_TYPES.TICK:
            return {
                ...
                status: state.secondsRemaining === 0 ? QUIZ_STATUS.FINISHED : state.status,
            };
...
export default function QuizProvider({ children }) {
    ...
    const { questions, allQuestions, status, index, answer, points, highscores, secondsRemaining, difficulty, userAnswers, isReviewing, reviewIndex } = state;
    ...
    useEffect(() => {
        async function fetchQuestions() {
            try {
                const res = await fetch(`${API_BASE_URL}/questions.json`);
